#!/usr/bin/env bash
set -e
which http xmllint ag fzy openssl jq > /dev/null
...
parse_http()
{
    http "$1" | xmllint --html --xpath "$2" - 2>/dev/null
}
...
ralode()
{
    ...
    __name=`jq -r '.[0] | .[] | .name' <<< "$__json" | fzy --prompt="Select a dubber: "`

    declare -A __items
    eval $(jq -r '.[0] | .[] | select(.name == "'"$__name"'") | .items[] | "__items[\"" + .sname + "\"]=" + (.scode | capture("src=(?<a>\"[^\"]+\")") | .a)' <<< "$__json")

    __episode=`printf "%s\n" "${!__items[@]}" | sort -V | tac | fzy --prompt="Select an episode: "`
